...
namespace Newtonic
{
  ...
  void Texture::LoadFromImage(const Image & image)
  {
    ...
    NW_WRAP_GL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
    NW_WRAP_GL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
